(function () {
  ...
  function organizationsController($q, uiGridConstants, dataservice, logger, $scope) {
    ...
    function attachDiscountCard(entity) {
      console.log(entity);
      $q.when(dataservice.attachDiscountCardsToOrganization(vm.currentOrganization.Id, [entity])).then(function () {
        getOrganizations();
      });
    }
    function attachCoupon(entity) {
      console.log(entity);
      $q.when(dataservice.attachCouponsToOrganization(vm.currentOrganization.Id, [entity])).then(function () {
        getOrganizations();
      });
    }
    function attachShop(entity) {
      console.log(entity);
      $q.when(dataservice.attachShopsToOrganization(vm.currentOrganization.Id, entity)).then(function () {
        getOrganizations();
      });
    }
    ...
    function deattachDiscountCard(entity) {
      console.log(entity);
      $q.when(dataservice.deattachDiscountCardsFromOrganization(vm.currentOrganization.Id, [entity])).then(function () {
        getOrganizations();
      });
    }
    function deattachCoupon(entity) {
      console.log(entity);
      $q.when(dataservice.deattachCouponsFromOrganization(vm.currentOrganization.Id, [entity])).then(function () {
        getOrganizations();
      });
    }
      function deattachShop(entity) {
        console.log(entity);
        $q.when(dataservice.deattachShopsFromOrganization(vm.currentOrganization.Id, [entity])).then(function () {
          getOrganizations();
        });
      } 
      function deattachImage(entity) {
        console.log(entity);
        $q.when(dataservice.removeImagesFromOrganization(vm.currentOrganization.Id, [entity])).then(function () {
          getOrganizations();
        });
      } 
      //Shops
      ...
      function deactivateCouponCodes(code) {
        $q.when(dataservice.deactivateCouponCodes([code.Id])).then(function (data) {
          vm.currentOrganization = dataservice.getOrganizationById(vm.currentOrganization.Id);
          vm.showCoupons(vm.currentOrganization)
